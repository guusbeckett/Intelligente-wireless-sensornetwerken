...
void XBee::send(XBeeRequest &request) {
        ...
        write(START_BYTE);				//sendByte(START_BYTE, false);

        // send length
        uint8_t msbLen = ((request.getFrameDataLength() + 2) >> 8) & 0xff;
        uint8_t lsbLen = (request.getFrameDataLength() + 2) & 0xff;

        write(msbLen);		//sendByte(msbLen, true);
        write(lsbLen);		//sendByte(lsbLen, true);

        // api id
        write(request.getApiId());			//sendByte(request.getApiId(), true);
        write(request.getFrameId());		//sendByte(request.getFrameId(), true);
...
//              std::cout << "sending byte [" << static_cast<unsigned int>(i) << "] " << std::endl;
                write(request.getFrameData(i));		//sendByte(request.getFrameData(i), true);
...
//      std::cout << "checksum is " << static_cast<unsigned int>(checksum) << std::endl;
        ...
        // send checksum
        write(checksum);		//sendByte(checksum, true);
